...
$run2 =mysqli_query($con,"update mdosa set qty=qty-1 where name='$name';");
$run3=mysqli_query($con,"update mdosa set qty=10 where qty=3;");

if($run1 && $run2)
{
	mysqli_commit($con);
	//$run3=mysqli_query($con,"update mdosa set qty=10 where qty=3;");
  header("LOCATION:menudosa.html");
}
else
{
	echo "Not inserted";
